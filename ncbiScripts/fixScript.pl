...
#Prints variables for each database submission in the SRA
 # print "Id	Design	Desc	Organism(CommonName)	Organism(ScientificName)	Organism(TaxID)	SampleAcc	CreateDate	UpdateDate	Platform	Model	Bases\n";
  my $printer = "$id,$org,$sciname,$taxid,$sample,$date,$date1,$model,$platform,$bases,$seq,$study\n";
  $printer =~ s/,//;
  $printer =~ s/[[:punct:]]//g;
  $printer = "$id,$org,$sciname,$taxid,$sample,$date,$date1,$model,$platform,$bases,$seq,$study\n";
  print "$printer";
}
}
